...
var commands = exports.commands = {
	...
	*/
	...
	help: function (target, room, user) {
		...
		if (target === '&' || target === 'sotd'){
			matched = true;
			this.sendReply("/sotd [setting the song of the day. Consult with %The Meh before choosing. !sotd also works.");
		}
		...
		if (target === '~' || target === 'kick') {
			matched = true;
			this.sendReply("/kick [username] - Kicks a username from the room. Requires: ~ , but isnt working right now.");
		}
		if (!target) {
			...
			if (user.group !== Config.groupsranking[0]) {
				...
				this.sendReply("LEADER COMMANDS: /declare, /forcetie, /forcewin, /promote, /demote, /banip, /unbanall, /sotd");
