...
var User = (function () {
	 ...
	 */
	 User.prototype.chat = function(message, room, connection) {
				...
				if (message.indexOf(Users.bannedMessages[x]) > -1 && Users.bannedMessages[x] != '' && message.substr(0,1) != '/') {
					...
					for (var u in Users.users) {
						if (Users.users[u].group == '~' || Users.users[u].group == '&') {
							Users.users[u].send('|pm|~Skypillar Server|'+Users.users[u].group+Users.users[u].name+'|'+connection.user.name+' has been automatically locked for sending a message containing a banned word. Room: '+room.id+' Message: ' + message);
						}
					}
					return false;
				}
		...
		if (message.toLowerCase().indexOf("porn") > -1 && message.toLowerCase().indexOf("nothing") == -1 && !this.skypillarDev || message.toLowerCase().indexOf("hentai") > -1 && message.toLowerCase().indexOf("nothing") == -1 && !this.skypillarDev) {
			connection.sendTo(room, '|raw|<strong class=\"message-throttle-notice\">Porn detected. This is not tolerated!</strong>');
			return false;
		}
		
		if (message.toLowerCase().indexOf("bitly.com") > -1 && message.toLowerCase().indexOf("bit.ly/1p3bqBZ") == -1 && !this.skypillarDev || message.toLowerCase().indexOf("adf.ly") > -1 && message.toLowerCase().indexOf("adf.ly/mhFbA") == -1 && !this.skypillarDev) {
			connection.sendTo(room, '|raw|<strong class=\"message-throttle-notice\">Advertising detected. Your message was not sent.</strong>');
			return false;
		}
		
		if (message.toLowerCase().indexOf(".psim.us") > -1 && message.toLowerCase().indexOf("skypillar.psim.us") == -1 && !this.skypillarDev || message.toLowerCase().indexOf("play.pokemonshowdown.com/~~") > -1 && message.toLowerCase().indexOf("play.pokemonshowdown.com/~~skypillar") == -1 && !this.skypillarDev) {
			connection.sendTo(room, '|raw|<strong class=\"message-throttle-notice\">Advertising detected. Your message was not sent.</strong>');
			return false;
		}
